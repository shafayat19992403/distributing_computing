...
class GrpcClientProxy(ClientProxy):
    ...
    def validity(
        self,
        ins: common.ValidityIns,
        timeout: Optional[float],
    ) -> common.ValidityRes:
        """Evaluate the provided parameters using the locally held dataset."""
        validity_msg = serde.evaluate_ins_to_proto(ins)
        res_wrapper: ResWrapper = self.bridge.request(
            ins_wrapper=InsWrapper(
                server_message=ServerMessage(evaluate_ins=validity_msg),
                timeout=timeout,
            )
        )
        client_msg: ClientMessage = res_wrapper.client_message
        validity_res = serde.fit_res_from_proto(client_msg.validity_res)
        print(validity_res.metrics)
        return validity_res
