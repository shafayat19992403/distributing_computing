...
from .numpy_client import has_validity as numpyclient_has_validity
...
def _validity(self: Client, ins: ValidityIns) -> ValidityRes:
    """Evaluate the provided parameters using the locally held dataset."""
    parameters: NDArrays = parameters_to_ndarrays(ins.parameters)

    results = self.numpy_client.validity(parameters, ins.config)  # type: ignore
    if not (
        len(results) == 1
        and isinstance(results[0], int)
    ):
        raise Exception(EXCEPTION_MESSAGE_WRONG_RETURN_TYPE_FIT)
...
def _wrap_numpy_client(client: NumPyClient) -> Client:
    ...
    if numpyclient_has_validity(client=client):
        member_dict["validity"] = _validity
