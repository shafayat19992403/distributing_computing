...
class Client(ABC):
    ...
    def validity(self, ins: ValidityIns) -> ValidityRes:
        """Refine the provided parameters using the locally held dataset.
        ...
        return ValidityRes(
            status=Status(
                code=Code.FIT_NOT_IMPLEMENTED,
                message="Client does not implement `validity`",
            ),
            validity=0,
        )
...
def has_validity(client:Client) -> bool:
    """Check if Client implements evaluate."""
    return type(client).validity != Client.validity
...
def maybe_call_validity(client: Client, validity_ins: ValidityIns) ->ValidityRes:
    """Call `evaluate` if the client overrides it."""
    # Check if client overrides `evaluate`
    if not has_validity(client=client):
        # If client does not override `evaluate`, don't call it
        status = Status(
            code=Code.EVALUATE_NOT_IMPLEMENTED,
            message="Client does not implement `validity`",
    ...
    # If the client implements `evaluate`, call it
    return client.validity(validity_ins)
