...
class RayClientProxy(ClientProxy):
    ...
    def validity(
        self, ins: common.ValidityIns, timeout: Optional[float]
    ) -> common.ValidityRes:
        """Evaluate model parameters on the locally held dataset."""
        future_evaluate_res = launch_and_evaluate.options(  # type: ignore
            **self.resources,
...
class RayActorClientProxy(ClientProxy):
    ...
    def validity(
        ...
        def validity() -> common.ValidityRes:
            client: Client = _create_client(self.client_fn, self.cid)
            return maybe_call_validity(
                client=client,
                validity_ins=ins,
            )

        res = self._submit_job(validity, timeout)
...
@ray.remote
def launch_and_validity(
    client_fn: ClientFn, cid: str, evaluate_ins: common.ValidityIns
) -> common.FitRes:
    """Exectue evaluate remotely."""
    client: Client = _create_client(client_fn, cid)
    return maybe_call_validity(
        client=client,
        validity_ins=evaluate_ins,
    )
